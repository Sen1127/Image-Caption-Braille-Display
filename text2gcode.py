...
def pytoscad(filename, originaltxt):
    ...
    print(f"len:{quantity_to_translate}")
...
def conversion(filename):
    ...
    for f in files:
        if f == f"{filename}.scad":                                   
            stl_file = f.replace('.scad', '.stl') 
            gcode_file = f.replace('.scad', '.gcode')
            printer_profile = 'Elegoo.ini'

            cmd = ["openscad-2021.01\\openscad.exe", "--export-format=binstl", "--render", "-o", stl_file, f]
            subprocess.run(cmd)
            cmd = ["Prusa3D\\PrusaSlicer\\prusa-slicer-console.exe", "--load", printer_profile, "--export-gcode", "--output", gcode_file, stl_file]
            subprocess.run(cmd)
